...
class Scanner (FileManager, ViewsManager):
    ...
    def __init__(self, ports=65535, breaks = 500, sleep=0, length_bar = 70, progress_container="█", path_file = "log.txt") -> None:
    ...
    async def _scan_port(self, ip_address:str, port:int) -> (dict or None):
        ...
        try:
            ...
            if self.SAVE_DATA:
                self.write_file(self.PATH_FILE, f"{ip_address}:{port} - {name} - {status} - {code}\n")
            return {
                "port": port,
                "name": name,
                "code": code,
    ...
    async def run (self) -> (str or None):
        ...
        start_time = timeit.default_timer() # Obten la fecha actual que servira para contar el tiempo de ejecución del programa
        ...
        for value in range(0, self.PORTS, self.BREAKS):
            ...
            for port in range(value, value + self.BREAKS):
                tasks.append(asyncio.create_task(self._scan_port(IP_ADDRESS, port)))
            ...
            self.progress_bar(self.PORTS, len(completed_tasks), self.LENGTH_BAR, self.PROGRESS_CONTAINER, f"Block: {counter_blocks}")

        end_time = timeit.default_timer() # Obten el tiempo al finalizar todos los puertos
        for task in tasks: task.cancel() # Una vez el proceso de escaneo haya finalizado eliminaremos las tareas
